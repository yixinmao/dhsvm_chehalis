...
PIXMET MakeLocalMetData(int y, int x, MAPSIZE * Map, int DayStep,
			...
			METLOCATION * Stat, uchar * MetWeights, uchar * WeightStn,
...
////////////////////////////////////////////////////////////////////////////////////
{
  ...
  int stn_ind;		/* station index to be used for interpolation in VERCRESS method */
...
/////////////////////////////// hacked by Yixin ////////////////////////////////////////
    if (Options->Interpolation == VARCRESS){
	    WeightSum = 0.0;
		for (i=0; i<Options->CressStations; i++) {
			WeightSum += (float) MetWeights[i];
		}
	    for (i = 0; i < NStats; i++) {
	    ...
	    for (i = 0; i < Options->CressStations; i++) {
		  ...
		  LapseT(Stat[stn_ind].Data.Tair, Stat[stn_ind].Elev, LocalElev,
		       Stat[stn_ind].Data.TempLapse);
	      LocalMet.Rh += CurrentWeight * Stat[stn_ind].Data.Rh;
	      if (Options->WindSource == STATION)
		  LocalMet.Wind += CurrentWeight * Stat[stn_ind].Data.Wind;
	      LocalMet.Lin += CurrentWeight * Stat[stn_ind].Data.Lin;
	      LocalMet.Sin += CurrentWeight * Stat[stn_ind].Data.Sin;
	      if (Options->Shading == TRUE) {
		    LocalMet.SinBeam += CurrentWeight * Stat[stn_ind].Data.SinBeamObs;
		    LocalMet.SinDiffuse += CurrentWeight * Stat[stn_ind].Data.SinDiffuseObs;
	      }
	      TempLapseRate += CurrentWeight * Stat[stn_ind].Data.TempLapse;
	    }
	  ...
	  if (Options->Interpolation == VARCRESS) {
	      for (i = 0; i < Options->CressStations; i++) {
		    ...
		    if (Options->PrecipLapse == MAP)
		      ...
		      LapsePrecip(Stat[stn_ind].Data.Precip, 0, 1, PrecipLapseMap[y][x]);
			else
			  PrecipMap->Precip += CurrentWeight *
		      LapsePrecip(Stat[stn_ind].Data.Precip, Stat[stn_ind].Elev, LocalElev,
			  Stat[stn_ind].Data.PrecipLapse);
	      }
	  }
	  else {
	      for (i = 0; i < NStats; i++) {
	  ...
	  if (Options->Interpolation == VARCRESS) {
	      for (i = 0; i < NStats; i++) {
			...
			if (Options->Outside == FALSE)
		      PrecipMap->Precip += CurrentWeight * Stat[stn_ind].Data.Precip /
			  PrismMap[Stat[stn_ind].Loc.N][Stat[stn_ind].Loc.E] * PrismMap[y][x];
			else
			  PrecipMap->Precip += CurrentWeight * Stat[stn_ind].Data.Precip /
			  Stat[stn_ind].PrismPrecip[Month - 1] * PrismMap[y][x];
			if (PrismMap[y][x] < 0){

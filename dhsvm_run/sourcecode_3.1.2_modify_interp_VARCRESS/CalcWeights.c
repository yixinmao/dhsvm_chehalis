...
void CalcWeights(METLOCATION * Station, int NStats, int NX, int NY,
		 uchar ** BasinMask, uchar **** WeightArray, uchar **** WeightStnInd,
...
//////////////////////////// hacked by Yixin ////////////////////////
  if (!((*WeightStnInd) = (uchar ***) calloc(NY, sizeof(uchar **))))
    ReportError("CalcWeights()", 1);

  for (y = 0; y < NY; y++)
    if (!((*WeightStnInd)[y] = (uchar **) calloc(NX, sizeof(uchar *))))
      ReportError("CalcWeights()", 1);

  for (y = 0; y < NY; y++) 
    for (x = 0; x < NX; x++) {
      if (!((*WeightArray)[y][x] = (uchar *) calloc(Options->CressStations, sizeof(uchar))))
	ReportError("CalcWeights()", 1);
      if (!((*WeightStnInd)[y][x] = (uchar *) calloc(Options->CressStations, sizeof(uchar))))
	ReportError("CalcWeights()", 1);
    }
////////////////////////////////////////////////////////////////////
...
//////////////////////////// hacked by Yixin ///////////////////////////
	  ...
	  for (i = 0, Denominator = 0; i < NStats; i++) {
		...
		(crt * crt + Distance[i] * Distance[i]);
		  (*WeightStnInd)[y][x][i] = stationid[i];
	      Denominator += InvDist2[i];
	    }
	    else
	      break;
